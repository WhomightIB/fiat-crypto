...
c=0,b=a.length;for(;;){const{read:d,written:e}=A.encodeInto(a.slice(c),L);b-=d;if(!b)return e;x(e);c+=d}},ta_create:(a,b)=>new
...
f=0;f<e;f++)c[d+f]=O(a,b+f)},ta_blit_to_string:(a,b,c,d,e)=>{for(let
f=0;f<e;f++)P(c,d+f,a[b+f])},wrap_callback:a=>function(){var
...
b==="string"?b:l.decode(b.slice(c,c+d))),d),read:(a,b,c,d,e)=>f.readSync(a,b,c,d,e),file_size:a=>f.fstatSync(a,{bigint:true}).size,register_channel:M,unregister_channel:Q,channel_list:z,exit:a=>h&&d.exit(a),argv:()=>h?d.argv.slice(1):["a.out"],getenv:a=>h?d.env[a]:null,system:a=>{var
b=require("node:child_process").spawnSync(a,{shell:true,stdio:"inherit"});if(b.error)throw b.error;return b.signal?255:b.status},time:()=>performance.now(),getcwd:()=>h?d.cwd():"/static",chdir:a=>d.chdir(a),mkdir:(a,b)=>f.mkdirSync(a,b),unlink:a=>f.unlinkSync(a),readdir:a=>f.readdirSync(a),file_exists:a=>+f.existsSync(a),rename:(a,b)=>f.renameSync(a,b),throw:a=>{throw a},start_fiber:a=>s(a),suspend_fiber:n({parameters:["externref","funcref","eqref"],results:["eqref"]},(c,b)=>new
...
a==="string"),compare:(a,b)=>a<b?-1:+(a>b),hash:E,decodeStringFromUTF8Array:()=>"",encodeStringToUTF8Array:()=>0},g=Object.assign({Math:J,bindings:v,js:u,"wasm:js-string":m,"wasm:text-decoder":m,"wasm:text-encoder":m,env:{}},C),r={builtins:["js-string","text-decoder","text-encoder"]};function
I(a){const
b=require("node:path"),c=b.join(b.dirname(require.main.filename),a);return require("node:fs/promises").readFile(c)}const
q=globalThis?.document?.currentScript?.src;function
B(a){const
b=q?new
URL(a,q):a;return fetch(b)}const
...
R=await
F();var{caml_callback:c,caml_alloc_tm:o,caml_start_fiber:y,caml_handle_uncaught_exception:p,caml_buffer:w,caml_extract_string:x,string_get:O,string_set:P,_initialize:k}=R.instance.exports,i=w?.buffer,L=i&&new
Uint8Array(i,0,i.length);s=n({parameters:["eqref"],results:["externref"]},y,{promising:"first"});var
...
k()})(function(a){"use strict";return{}}(globalThis))({"link":[["code-ee9fe545bfa17cec86dc",0]],"generated":(a=>{var
